...
//     const KRemaining_Quantity = Total; // Set Remaining_Quantity in KensaDB to Total
...
//     // Calculate Total_NG
//     const KtotalNG = counter1+counter2+counter3+counter4+counter5+counter6+counter7+counter8+counter9+counter10+counter11+counter12;
...
//     // Save to SRSDB
//     const saveResponse = await fetch('http://localhost:3000/submitTokensaDB', {
//       method: 'POST',
...
//Scan Button for new KENSA
//Scan Button for Kensa
  ...
  html5QrCode
    .start(
      ...
      async (qrCodeMessage) => {
        ...
        try {
          const response = await fetch("http://localhost:3000/processKensa", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
          ...
          if (data.source) {
            ...
            console.log("Kensa data processed successfully!");
